<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!-- header -->
    <header id="header">
        <div class="g-navi sp">
            <a class="sp" href="/index.html">
                <div class="logo">
                    <img class="logo-img" src="img/logo/logo_mini.png" alt="logo">
                </div>
            </a>

            <div class="hamburger sp">
                <div class="hamburger-wrap">
                    <i class="burger"></i>
                    <i class="burger"></i>
                    <i class="burger"></i>
                </div>
            </div>
        </div>

        <div class="g-navi-wrap">
            <div class="g-navi pc">
                <a class="g-navi-items" href="/index.html">
                    <div style="position: relative;">
                        <div class="flexR">
                            <i class="material-symbols-outlined">map</i>
                            <p class="map">マップ検索</p>
                            <div class="navi-mark">
                                <img src="img/index/gnav/pc/nav_map.png" alt="map-search">
                            </div>
                        </div>
                    </div>
                </a>

                <a class="g-navi-items" href="/search.html">
                    <div class="flexR">
                        <i class="material-symbols-outlined">search</i>
                        <p class="category">　検索</p>
                        <img class="navi-mark" src="img/index/gnav/pc/nav_search.png" alt="search">
                    </div>
                </a>

                <a class="g-navi-items" href="/feature_top.html">
                    <div class="flexR">
                        <i class="material-symbols-outlined">topic</i>
                        <p class="recommend">　特集</p>
                        <img class="navi-mark" src="img/index/gnav/pc/nav_pickup.png" alt="recommend">
                    </div>
                </a>
            </div>
            <div class="g-navi tab">
                <a href="/index.html">
                    <div class="navi-mark">
                        <img src="img/index/gnav/tab/nav_map_tab.png" alt="map-search">
                    </div>
                </a>

                <a href="/search.html">
                    <img class="navi-mark" src="img/index/gnav/tab/nav_search_tab.png" alt="search">
                </a>

                <a href="/feature_top.html">
                    <img class="navi-mark" src="img/index/gnav/tab/nav_pickup_tab.png" alt="recommend">
                </a>
            </div>

            <div class="site-map sp">
                <div id="container" class="container">
                    <div class="site-map-title center">みちのえき<br>サイトマップ</div>
                    <br>
                    <hr>
                    <div class="wrap">
                        <a class="item item1 flexR" href="/index.html">
                            <div class="flexC">
                                <i class="material-symbols-outlined icon">map</i>
                                マップ検索
                            </div>
                        </a>
                        <a class="item item2 flexR" href="/search.html">
                            <div class="flexC">
                                <i class="material-symbols-outlined icon">search</i>
                                カテゴリー検索
                            </div>
                        </a>
                        <a class="item item3 flexR" href="/feature_top.html">
                            <div class="flexC">
                                <i class="material-symbols-outlined icon">topic</i>
                                特集
                            </div>
                        </a>
                        <div class="item item4 flexR">
                            <div class="flexC">
                                <i class="fa-brands fa-instagram icon"></i>
                                インスタグラム
                            </div>
                        </div>
                        <a class="item item5 flexR" href="/inquiry.html">
                            <div class="flexC">
                                <span class="material-symbols-rounded icon">outgoing_mail</span>
                                お問い合わせ
                            </div>
                        </a>
                        <a class="item item6 flexR" href="/white_paper.html">
                            <div class="flexC">
                                <span class="material-symbols-rounded icon">help</span>
                                このHPについて
                            </div>
                        </a>
                    </div>
                    <hr>
                    <br>
                    <p class="center">Copyright All Rights Reserved Team 繁</p>
                </div>
            </div>
        </div>
    </header>
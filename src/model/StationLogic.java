...
public class StationLogic {
	...
	public List<Station> getAllStation(int showflag, int con_id) {
		//DAOオブジェクト生成
		M_road_stationsDAO dao = new M_road_stationsDAO();
		//DBから全レコードを取得する
		List<Station> StationList = dao.getStation(showflag,con_id);
		return StationList;
	}

	//指定された道の駅IDのおススメ情報を表示する
	public List<Station> getAllRecommends(int showflag, int con_id) {
		//DAOオブジェクト生成
	...
	public List<Station> getAllSpots(int showflag, int con_id) {
		//DAOオブジェクト生成
	...
	public List<Station> getAllFacilities(int showflag, int con_id) {
		//DAOオブジェクト生成
